...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	init_frm();
	pt_t *pt_entry;
 	pd_t *pd_entry = FRAME0 * NBPG;
 	
 	//allocating page directory
	frm_tab[0].fr_status = FRM_MAPPED;
        frm_tab[0].fr_type = FR_DIR;
        frm_tab[0].fr_pid = NULLPROC;
        proctab[NULLPROC].pdbr = (FRAME0 + 0);	

	i=1;
	while(i < 5) 
	{
		//initializing page directory entry
		pd_entry->pd_pres = 1;
	        pd_entry->pd_write = 1;
	        pd_entry->pd_user = 0;
	        pd_entry->pd_pwt = 0;
		...
		pt_entry = (FRAME0 + i) * NBPG;
		
		j=0;
		while(j < 1024) 
		{
			//initializing page table entry
			pt_entry->pt_pres = 1;
			pt_entry->pt_write = 1;
			pt_entry->pt_user = 0;
			pt_entry->pt_pwt = 0;
	...
	write_cr3(proctab[NULLPROC].pdbr*NBPG);
	enable_paging();
	return(OK);
}
